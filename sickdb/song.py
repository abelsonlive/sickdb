...
class Song(object):
    ...
    def get_id3_tags(self):
        ...
        if self.attrs["tags"].get("bit_rate", None) is None:
            ...
            self.attrs["tags"]["artist"] = util.std_string(
                util.unlistify(self.id3.tags.get("ARTIST", "")))
            self.attrs["tags"]["title"] = util.std_string(
                util.unlistify(self.id3.tags.get("TITLE", "")))
            self.attrs["tags"]["album"] = util.std_string(
                util.unlistify(self.id3.tags.get("ALBUM", "")))
            self.attrs["tags"]["date"] = util.std_string(
                util.unlistify(self.id3.tags.get("DATE", "")))
            self.attrs["tags"]["bpm"] = util.std_string(
                util.unlistify(self.id3.tags.get("BPM", "")))
            raw_key = util.unlistify(self.id3.tags.get("INITIALKEY", ""))
            self.attrs["tags"]["initialkey"] = settings.HARMONIC_TO_KEY.get(
                raw_key, settings.KEY_LOOKUP.get(raw_key, ""))
    ...
    def get_fingerprint(self):
        ...
        if self.attrs.get("fingerprint", None) is None:
            cmd = '{0} {1} -json'.format(settings.FP_CALC_PATH,
                                         pipes.quote(self.file))
            p = util.sys_exec(cmd)
            if not p.ok:
                sys.stderr.write(
                    "WARNING: Error running {0}: {1}\n".format(cmd, p.stderr))
            try:
                data = json.loads(p.stdout)
                data["duration"] = str(int(round(data.get("duration", 0), 0)))
                data["uid"] = hashlib.md5(
                    data.get("fingerprint", "").encode("utf-8")).hexdigest()
                self.attrs.update(data)
            except Exception as e:
                sys.stderr.write(
                    "WARNING: Could not compute fingerprint for {0}\n".format(self.file))
                self.attrs.update({"fingerprint": "", "duration": "", "uid": hashlib.md5(
                    self.file.encode("utf-8")).hexdigest()})
    ...
    def get_bpm_and_key(self):
        ...
        if self.attrs.get("bpm", None) is None:
            ...
            # cmd
            cmd = '{0} {1} "{2}"'.format(
                settings.FREESOUND_PATH, pipes.quote(self.file), fp_root)
            ...
            try:
                ...
                sys.stderr.write(
                    "WARNING: Could not decode {0} because: {1}".format(self.file, e))
                return
            # remove
            try:
                os.remove(fp_root)
            ...
            self.attrs["bpm"] = str(
                round(data.get("rhythm", {}).get("bpm", settings.DEFAULT_BPM), 1))
            self.attrs["key"] = settings.KEY_LOOKUP.get((data.get("tonal", {}).get("key_temperley", {}).get(
                "key", "") + data.get("tonal", {}).get("key_temperley", {}).get("scale", "")).upper(), settings.DEFAULT_KEY)
            self.attrs["chord"] = settings.KEY_LOOKUP.get((data.get("tonal", {}).get(
                "chords_key", "") + data.get("tonal", {}).get("chords_scale", "")).upper(), settings.DEFAULT_KEY)
            self.attrs["harmonic_key"] = settings.KEY_TO_HARMONIC.get(self.attrs[
                                                                      "key"], "")
        ...
        if self.attrs["musicbrainz"].get("id", None) is None:
            ...
            if res["status"] == "error":
                sys.stderr.write("WARNING: Could not resolve {0} with Musicbrainz because: {1}\n".format(
                    self.file, res["error"]["message"]))
                rec = {}

            res = res.get("results", [])
            if not len(res):
                sys.stderr.write(
                    "WARNING: Could not find Musicbrainz info for {0}\n".format(self.file))
                rec = {}
    ...
    def reconcile(self):
        ...
        if self.attrs["musicbrainz"].get("artist"):
            self.truth["artist"] = util.std_string(
                self.attrs["musicbrainz"]["artist"])
        elif self.attrs["tags"].get("artist"):
            self.truth["artist"] = util.std_string(
                self.attrs["tags"]["artist"])
        elif self.attrs["file"].get("artist"):
            self.truth["artist"] = util.std_string(
                self.attrs["file"]["artist"])

        # analyze ALBUM
        if self.attrs["tags"].get("album"):
            self.truth["album"] = util.std_string(
                self.attrs["tags"]["album"].strip())
        elif self.attrs["file"].get("album"):
            self.truth["album"] = util.std_string(
                self.attrs["file"]["album"].strip())

        # analyze TITLE
        if self.attrs["musicbrainz"].get("title"):
            self.truth["title"] = util.std_string(
                self.attrs["musicbrainz"]["title"])
        ...
        self.truth["directory"] = os.path.join(
            settings.DIR_FORMAT.format(**self.truth))
        self.truth["file"] = os.path.join(
            self.truth["directory"], settings.FILE_FORMAT.format(**self.truth))
