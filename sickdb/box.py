...
class Box(object):
    ...
    def update_song(self, song):
        ...
        if song.attrs["uid"] in self.uids:
            sys.stderr.write("INFO: Found Duplicate: {0}".format(song.file))
            self.duplicates.append(song)
        else:
    ...
    def import_song(self, song):
        ...
        truth_file = os.path.join(self.directory, song.truth[
                                  "file"]).replace('//', '/')
        if song.file != truth_file:
            p = util.sys_exec(
                'mkdir -p "{0}"'.format(self.directory + song.truth["directory"]))
            if not p.ok:
                sys.stderr.write("ERROR: Could not copy file from {0} to {1} because {2}\n".format(
                    song.file, truth_file, p.stdout))
            sys.stderr.write(
    ...
    def dedupe(self):
        ...
        sys.stderr.write("INFO: Deduping {0} files from SickDB {1}\n".format(
            self.num_duplicates, self.directory))
        for song in self.duplicates:
            sys.stderr.write(
                "INFO: Removing Duplicate: {0}\n".format(song.file))
            try:
                os.remove(song.file)
            except:
                pass
